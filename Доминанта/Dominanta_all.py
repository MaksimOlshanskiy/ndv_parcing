import datetime
import time
import requests

from functions import save_flats_to_excel
from save_to_excel import save_flats_to_excel_old_new

"""

Все проекты, если появляется новый проект, то внести его в в список projects_ids
"""

projects_ids = ['hide', 'hideout', 'svet']
projects_dict = {'HIDE' : 'Hide', 'hideOUT' : 'Hide out', 'svet' : 'Свет'}
flats = []

for project_id in projects_ids:

    params = {
        'filter[price][0]': '0',
        'filter[price][1]': '0',
        'filter[sq][0]': '0',
        'filter[sq][1]': '0',
        'filter[profile]': 'Жилая',
        'filter[project_code]': project_id,
        'filter[type]': '',
        'filter[price_mlnusd][0]': '0',
        'filter[price_mlnusd][1]': '0',
        'filter[price_mlneur][0]': '0',
        'filter[price_mlneur][1]': '0',
        'filter[price_sqm][0]': '0',
        'filter[price_sqm][1]': '0',
        'filter[price_sqmusd][0]': '0',
        'filter[price_sqmusd][1]': '0',
        'filter[price_sqmeur][0]': '0',
        'filter[price_sqmeur][1]': '0',
        'filter[hide_reserved][0]': 'Y',
        'filter[flat]': '',
        'sort[price]': '1',
        'page': '1',
        'cnt': '500'
    }


    count = 0

    while True:

        response = requests.get("https://d-a.ru/ajax/flats/", params=params)

        if response.status_code != 200:
            print(f'Ошибка: {response.status_code}')
            break

        item = response.json()
        items = item.get("data", [])

        if not items:
            print("Данные закончились, выходим из цикла.")
            break

        for i in items:
            count += 1
            date = datetime.date.today()
            raw_name = i['project']['name']
            project = projects_dict.get(raw_name, raw_name)
            developer = "Доминанта"
            korpus = i["building"]
            finish_type = 'Предчистовая'
            room_count = int(i.get("bedroomscount", ""))
            area = float(i.get("sq", ""))
            price = int(i.get("price", "").replace(' ', ''))
            old_price = int(i.get("priceold", ""))
            section = i.get("section", "")
            floor = int(i.get("floor", ""))

            if old_price == 0:
                old_price = price
                price = None

            print(
                f"{count} | {project}, дата: {date}, комнаты: {room_count}, площадь: {area}, цена: {price}, корпус: {korpus}, этаж: {floor}"
            )

            flats.append([date, project, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", developer, "",
                          "", "", "", korpus, "", "", "", "", "", "", "квартиры", finish_type, room_count, area, "",
                          old_price,
                          "", '', price, section, floor, ""])

        params['page'] = str(int(params['page'])+1)


        time.sleep(0.3)

save_flats_to_excel(flats, project, developer)
print(f"Файл сохранен")
