...
flats = []
lot_types = ['townhouses', 'ihc', 'flats']
for lot_type in lot_types:
    ...
    while True:
        ...
        for i in items:
            ...
            project = (i["project"]["name"].replace('МИRA', 'Мира').replace('МЫС – премиальный загородный проект', 'МЫС').replace('ДЖОЙС', 'Джойс').replace('ФЕЙМОС', 'Феймос').replace('МОД', 'Мод').
                       replace('СЛАВА', 'Слава').replace('ФОРУМ', 'Форум').replace('Мыс. Таунхаусы', 'Мыс').replace('Мыс. Коттеджи', 'Мыс'))
            english = ''
            promzona = ''
            mestopolozhenie = ''
            subway = ''
            ...
            stadia = ''
            dogovor = ''
            type= i['subtype']['name'].replace('ИЖС ', 'Коттеджи')
            try:
