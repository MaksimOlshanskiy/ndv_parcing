...
while len(flats) < total_count:
    ...
    for i in items:
        ...
        try:
            ...
            okrug = ''
        try:
            rayon = i['geo']['address'][2]['name']
        except:
            rayon = ''
        metro = i['geo']['address'][3]['name']
        try:
        ...
        print(
            f"{url}, дата: {date}, площадь: {area}, цена: {price}, {okrug}, {rayon}, {metro} ")
        result = [date, url, adress, okrug, rayon, metro, area, price, rooms]
        flats.append(result)
...
filename = f"Аренда_ЦАО_4_{current_date}.xlsx"
