        ...
        for room_id in rooms_ids:
            ...
            for f in total_floor_list:
                ...
                while True:
                    ...
                    async def fetch_page(session, json_data):
                        ...
                        async with sem:
                            async with session.post(URL, json=json_data) as response:
                                ...
                                items = data["data"]["offersSerialized"]

                                return items


                    async def main(all_json_queries):

                        async with aiohttp.ClientSession(headers=headers, cookies=cookies) as session:
                            tasks = []

                            for json_data in all_json_queries:
                                tasks.append(fetch_page(session, json_data))
                    ...
                    for page in range(1, 200):
                        ...
                        all_json_queries.append(json_data)

                    data = asyncio.run(main(all_json_queries))
