...
folder_path = r"C:\Users\m.olshanskiy\Desktop\База февраль 2026\Новая папка"

# Сохраняем объединённые данные в новый Excel файл
output_file_name = 'Пермь.xlsx'
...
all_data['Корпус'] = all_data['Корпус'].apply(
    lambda x: str(x).replace(',', '.') if pd.notna(x) else x
)
all_data['Корпус'] = all_data['Корпус'].apply(
    lambda x: str(int(float(x))) if str(x).endswith('.0') else x
)
