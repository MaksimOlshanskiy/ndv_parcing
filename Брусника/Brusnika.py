...
params = {
    ...
    'is_group': '0',
    "purchase_methods": "3",   # способ оплаты. 3 - полная оплата
}
...
for i in items:
    ...
    print(
        f"{project}, {url}, дата: {date}, тип: {room_count}, площадь: {area}, цена: {price}, старая цена: {old_price}, корпус: {korpus}, этаж: {floor}, {finish_type} ")
...
base_path = r"/Брусника"

folder_path = os.path.join(base_path, str(current_date))
if not os.path.exists(folder_path):
    os.makedirs(folder_path)

filename = f"Brusnika_{current_date}.xlsx"
