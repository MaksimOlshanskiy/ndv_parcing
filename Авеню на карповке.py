...
url = f'https://avenuenakarpovke.ru/select-parameters/'

driver.get(url=url)
page_content = driver.page_source  # Получаем HTML страницы после полной загрузки JavaScript
soup = BeautifulSoup(page_content, 'html.parser')
lots = soup.select("a.property-line.property-line-show")
...
for i in lots:
    ...
    url = ''
    developer = "Электрон"
    project = 'Авеню на Карповке'
    korpus = i.get('data-tower')
    type = 'Квартира'
    finish_type = 'Без отделки'
    room_count = i.get('data-rooms')
    try:
        area = i.get("data-area")
    except:
        area = ''
    try:
        old_price = i.get("data-cost")
    except:
        old_price = ''
    try:
        price = i.get("data-cost")
    except:
        price = ''
    section = ''
    try:
        floor = i.get("data-floor")
    except:
        floor = ''
    flat_number = ''
    english = ''
