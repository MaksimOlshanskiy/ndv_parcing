...
# Загружаем файлы
df1 = pd.read_excel(r"C:\Users\m.olshanskiy\Desktop\База декабрь\База изменяемые данные_NEWW.xlsx")
df2 = pd.read_excel(r"C:\Users\m.olshanskiy\Desktop\База декабрь\НашДомРФ.xlsx")
...
# Создаём столбец с количеством дней до срока на лету
merged["Дней_до_сдачи"] = merged["Срок сдачи"].apply(days_until_quarter)



mask_stage1 = (
    (merged['Стадия строительной готовности'] == 'нулевой цикл') &
    (merged['Дней_до_сдачи'].notna())
)
mask_stage2 = (
    (merged['Стадия строительной готовности'] == 'монтаж первых этажей') &
    (merged['Дней_до_сдачи'].notna())
)
mask_stage3 = (
    (merged['Стадия строительной готовности'] == 'монтаж средних этажей') &
    (merged['Дней_до_сдачи'].notna())
)
mask_stage4 = (
    (merged['Стадия строительной готовности'] == 'монтаж последних этажей') &
    (merged['Дней_до_сдачи'].notna())
)
...
merged.loc[mask_stage2, 'stage_2_date'] = (
    today + pd.to_timedelta((merged.loc[mask_stage2, 'Дней_до_сдачи'] / 2.5) / 2, unit='D')
)
merged.loc[mask_stage2, 'stage_3_date'] = (
    today + (pd.to_timedelta((merged.loc[mask_stage2, 'Дней_до_сдачи'] / 2.5) / 2, unit='D' ))
    + (pd.to_timedelta((merged.loc[mask_stage2, 'Дней_до_сдачи'] / 2.5), unit='D' )))
merged.loc[mask_stage3, 'stage_3_date'] = (
    today + pd.to_timedelta((merged.loc[mask_stage3, 'Дней_до_сдачи'] / 2), unit='D')
)
merged.loc[mask_stage4, 'stage_3_date'] = (
    today + pd.to_timedelta((merged.loc[mask_stage4, 'Дней_до_сдачи'] * 2 / 3 / 2), unit='D')
)


merged["Стадия строительной готовности"] = merged["Стадия строительной готовности"].replace({
    "нулевой цикл": "начальный цикл",
    "монтаж первых этажей": "начальный цикл",
    "монтаж средних этажей": "монтажные работы",
    "монтаж последних этажей": "монтажные работы",
    "ФОР": "завершающий цикл",
    "благоустройство": "завершающий цикл"
})


today = pd.Timestamp.today() + pd.Timedelta(days=35)  # текущая дата без времени

# Сохраняем результат
merged.to_excel(r"C:\Users\m.olshanskiy\Desktop\База декабрь\Тест222.xlsx", index=False)
