...
def save_flats_to_excel(flats, project, developer, kvartirografia=True, drop_columns=True):
    ...
    # заменяем NaN и 0 в "Цена со скидкой" на цену лота
    df['Цена кв.м, руб.'] = df['Цена лота, руб.'] / df['Площадь, кв.м']
    df['Цена со скидкой, руб.'] = df['Цена со скидкой, руб.'].replace(0, np.nan).replace(['', ' '], np.nan)
    df['Цена со скидкой, руб.'] = df['Цена со скидкой, руб.'].fillna(df['Цена лота, руб.'])


    # считаем скидку (доля)
    df['Скидка,%'] = 1 - (df['Цена со скидкой, руб.'] / df['Цена лота, руб.'])

    # на всякий случай приводим к float
    df['Скидка,%'] = df['Скидка,%'].astype(float).round(2)

    df['Цена кв.м со ск, руб.'] = df['Цена со скидкой, руб.'] / df['Площадь, кв.м']
