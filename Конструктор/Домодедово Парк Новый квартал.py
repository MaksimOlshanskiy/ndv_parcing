...
while True:
    ...
    url = f'https://www.domodedovograd.ru/domodedovo?grp=242602&page={page_counter}'

    driver.get(url=url)
    page_content = driver.page_source  # Получаем HTML страницы после полной загрузки JavaScript
    soup = BeautifulSoup(page_content, 'html.parser')
    flats_soup = soup.find_all('div', class_="product-card-v1")
    ...
    for i in flats_soup:
        ...
        try:
            ...
            old_price = ''
        try:
            for el in i.select('.product-card__price'):
                if el.get('class') == ['product-card__price']:
                    price = int(el.text.replace('₽', '').replace(' ', ''))
        except:
            price = ''
        section = ''
        try:
            floor = int(i.find(class_='product-card__subtitle').text.split()[4])
        except:
            floor = ''
        flat_number = int(i.find(class_='product-card__subtitle').text.split()[7])
        english = ''
        promzona = ''
        mestopolozhenie = ''
        subway = ''
