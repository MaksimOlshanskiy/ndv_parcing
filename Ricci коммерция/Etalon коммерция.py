...
while pagination['offset'] < number_of_flats:
    ...
    url = f'https://newsite.etalongroup.ru/api/filter/msk/commerce/list/'
    ...
    try:
        ...
        for i in items:
            ...
            print(
                f"{count}, {project}, дата: {date}, комнаты: {room_count}, площадь: {area}, цена: {price}, старая цена: {old_price}, этаж: {floor}, срок сдачи: {srok_sdachi_old}")

            result = [date, project, '', '', '', '', '', '', '', '', '', '', '', '', status, '', '', developer, okrug,
                      district, '', '', '', '', '', '', srok_sdachi_old, '', '', type, finish_type, room_count, area, '',
