import pandas as pd
import json

lower_words = {'на', 'в', 'и', 'с', 'к', 'у', 'о', 'от', 'по', 'за', 'из', 'над', 'под', 'об', 'же', 'ли', 'бы'}

def correct_title(text):
    if not isinstance(text, str):
        return text
    # Разбиваем на слова
    words = text.lower().split()
    # Первое слово всегда с большой буквы
    result = [words[0].capitalize()]
    # Остальные слова: если в списке исключений -> оставляем маленькими, иначе с большой
    for word in words[1:]:
        if word in lower_words:
            result.append(word)
        else:
            result.append(word.capitalize())
    return ' '.join(result)


# Загружаем данные из Excel файла
df = pd.read_excel(r"\\192.168.252.25\аналитики\ОТЧЕТЫ\Рабочие для базы\Квартирография_new.xlsx")  # укажите путь к вашему файлу

# Предполагаем, что колонки называются:
# 'Название проекта', 'Площадь', 'кол-во комнат'
# Если названия другие - замените их

# Приводим названия проектов к нижнему регистру
df['Название проекта'] = df['Название проекта'].apply(correct_title)

# 1. Приводим к текстовому типу (строковому)
df['Кол-во комнат'] = df['Кол-во комнат'].astype(str)

# 2. Заменяем латинскую e на русскую е
df['Кол-во комнат'] = df['Кол-во комнат'].str.replace('e', 'е', regex=False)



# Сортируем внутри каждого проекта по площади (если 'Площадь, кв.м' — числовая)
df = df.sort_values(by=['Название проекта', 'Площадь, кв.м'])

# Группируем данные по названию проекта
result = {}
for project in df['Название проекта'].unique():
    # Фильтруем данные для текущего проекта
    project_data = df[df['Название проекта'] == project]

    # Создаем словарь площадь: тип_комнаты
    area_rooms = {}
    for _, row in project_data.iterrows():
        area = str(row['Площадь, кв.м'])  # преобразуем в строку для ключа
        room_type = row['Кол-во комнат']
        area_rooms[area] = room_type

    result[project] = area_rooms

# Сохраняем в JSON файл
with open('output.json', 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print("Данные успешно сохранены в output.json")